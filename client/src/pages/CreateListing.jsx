...
export default function CreateListing() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if(formData.imageUrls.length < 1) return setError('You must uploar at least one image');
            if(+formData.regularPrice < +formData.discountPrice) return setError("Discount price can't be greater than regular price");
            setLoading(true);
            setError(false);
